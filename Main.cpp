...
unsigned int refreshLCDs(basicLCD** LCD, bool* redo, bool pause, int state, list<string>& twits, int twit_cursor, int scroll_speed);
								...
								if (al_install_mouse()) {
									...
									if (!!timerA) {
										...
										/*RUNNING STARTS HERE*/			while (running)
										{
											...
											while (al_get_next_event(queue, &ev))
											{
												...
												if (ev.type == ALLEGRO_EVENT_TIMER) {
													...
													if (state == DOWNLOADING) {
														if (!download_req && twitterDL.getStatus() == DOWNLOADING) {
															twitterDL.cancelDownload();
															curr_twit = 0;
															state = DISPLAYING;
														}
														else if (twitterDL.getStatus() != DOWNLOADING) {
															curr_twit = 0;
															state = DISPLAYING;
														}
													}
											...
											ImGui::SameLine();
											ImGui::SameLine(); ImGui::SliderInt(" ", &scroll_speed, MIN_SCROLL_SPEED, MAX_SCROLL_SPEED, NULL, reg_slider);		   // Slider for Twit quantity
											ImGui::SameLine();
											ImGui::SameLine(); HelpMarker(
												"LCD scrolling speed.\n"
											...
											if (refresh) {
												...
												curr_twit = refreshLCDs(LCD, &redo, pause, state, twitList, curr_twit, scroll_speed);
...
unsigned int refreshLCDs(basicLCD** LCD, bool* redo, bool pause, int state, list<string>& twits, int twit_cursor, int scroll_speed) {
	...
	list<string>::iterator listIterator = twits.begin();
	...
	if (state != IDLE) { //if IDLE, do nothing
		...
		int waiting = MAX_SCROLL_SPEED - scroll_speed;
		if (counter > waiting) {
			counter = 0;
			if (state == DOWNLOADING) {
				//Do download cycle
				if (previousState == DISPLAYING || previousState == IDLE)
				{
					offset = 0;
				}
				...
				for (int i = 0; i < 3; i++) {
					if (LCD[i] != nullptr)
					{
						...
						/*if(twits.screen_name.size()=<MAX_COL)
							...
							*LCD[i] << twitList.screen_name[offset];*/
				...
				for (size_t i = 0; i < curr_twit; i++)
				{
					listIterator++;
				}
				if (!pause) {
					if (curr_twit < twits.size()) {
						for (int i = 0; i < 3; i++) {
							if (LCD[i] != nullptr)
							{
								LCD[i]->lcdClear();
								*LCD[i] << (const unsigned char*)(*listIterator).substr(0, 16).c_str();
								LCD[i]->lcdSetCursorPosition(second_line);
								*LCD[i] << (const unsigned char*)(((*listIterator).substr(16, string::npos)).substr(offset, 16)).c_str();
								
							}
						}
						offset++;
						if (offset == ((*listIterator).substr(16, string::npos)).size()) {
							offset = 0;
							curr_twit++;
						}
					}
						...
						for (int i = 0; i < 3; i++) {
							if (LCD[i] != nullptr)
							{
								LCD[i]->lcdClear();
								LCD[i]->lcdSetCursorPosition(second_line);
								*LCD[i] << (const unsigned char*)"Ultimo Twit.";
							}
