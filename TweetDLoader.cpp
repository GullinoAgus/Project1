	...
	else
	{
		...
		if (this->status == IDLE)
		{
			...
			else
			{
				...
				try
				{
					receivedJSON = json::parse(this->receivedData);
				}
				catch (const std::exception&)
				{
					throw std::exception("Error with twitter response");
				}

				if (receivedJSON.is_object())
				{
					std::string errormessage;
					if (receivedJSON["errors"].is_array())
					{
						errormessage = receivedJSON["errors"][0]["message"];
					}
					else
					{
						errormessage = receivedJSON["error"];
					}
					receivedJSON.clear();
					throw std::exception(errormessage.c_str());
				}
				procesamiento(receivedJSON, buffer);
				return 0;
			}
		}
	}
